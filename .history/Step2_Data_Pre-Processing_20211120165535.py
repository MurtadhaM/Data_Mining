...
def clean_data(text):
    # Lowercase words
    reviews = [str(word).lower() for word in text]
    # Eliminate StopWords
    reviews = [stopwords_elimination(word) for word in text]
    # Eliminates Emails
    reviews = [re.sub(r'\S*@\S*\s?', '', str(review)) for review in text]
    # Eliminate new line
    reviews = [re.sub(r'\s+', ' ', str(review)) for review in text]
    # Eliminate single quotes
    reviews = [re.sub("\'", "", review) for review in text]
    # Eliminates words where len(word) < 3
    reviews = [' '.join([word for word in review.split() if len(word)>2]) for word in text]
    return clean_data
